...
{
	$q = $_GET['q'];
	$uname = $wpdb->get_var("SELECT username FROM {$table_prefix}users WHERE user_login = '$q'");
	if($uname)
...
else if($action == 'sponsor')
{
	...
	$q = $_GET['q'];
	$sname = $wpdb->get_var("SELECT username FROM {$table_prefix}users WHERE `user_login` = '$q'");
