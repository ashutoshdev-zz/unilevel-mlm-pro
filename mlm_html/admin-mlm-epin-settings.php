...
<p><strong><?php _e('No. of ePins', 'unilevel-mlm-pro'); ?></strong> - <?php _e('Specify the number of ePins to generate.', 'unilevel-mlm-pro') ?></p>
    ...
    <?php $selected = 'selected="selected"'; ?>
                                <select name="epins_type" id="epins_type" onchange="epinType(this.value);">
                                    <option value="1" <?php if (isset($_POST['epins_type']) && $_POST['epins_type'] == '1')
    {
        echo $selected;
    } else
    {
        echo '';
    } ?>><?php _e("Regular", "unilevel-mlm-pro") ?></option>
                                    <option value="0" <?php if (isset($_POST['epins_type']) && $_POST['epins_type'] == '0')
    {
        echo $selected;
    } else
    {
        echo '';
    } ?>><?php _e("Free", "unilevel-mlm-pro") ?></option>
                                </select>
                            </td>
                            <td>&nbsp;</td>
                        </tr>
                        ...
                        <tr>
                            <td><strong><?php _e('ePin Value/Product', 'unilevel-mlm-pro'); ?></strong></td>
                            <td>:&nbsp;</td>
                            <td><select name="epins_value" id="epins_value">
<?php foreach($pro_price_settings as $pricedetail){ ?>       
<option value="<?php echo $pricedetail->p_id ?>"><?php echo $pricedetail->product_name.' ('.$mlm_settings['currency'].' '.$pricedetail->product_price.')' ?></option>
<?php } ?>
</select>
                            </td>
                            <td>&nbsp;</td>
