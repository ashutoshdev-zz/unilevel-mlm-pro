...
function register_user_html_page(){
	...
	$user_role = array_shift($user_roles);
		...
		if(!empty($_GET['sp_name']))
	       ...
	       ?>
		 <script>$.cookie('s_name','<?php  echo   $sp_name ?>',{ path: '/' });</script>
...
function checkePinAvailability(str)
{
	...
	xmlhttp.open("GET", "<?php  echo   plugins_url().'/'.MLM_PLUGIN_NAME.'/ajax/check_epin.php'?>"+"?q="+str,true);
	xmlhttp.send();
     }
}
...
function checkePinAvailability1(str)
{
	...
	xmlhttp.open("GET", "<?php  echo   plugins_url().'/'.MLM_PLUGIN_NAME.'/ajax/check_epin.php'?>"+"?r="+str,true);
	xmlhttp.send();
      }
}
</script>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<form name="frm" method="post" action="" onSubmit="return formValidationNewVer();">
		...
		<?php }
                ...
                <tr><td colspan="2">&nbsp;</td></tr>
		<tr>
			<td><?php _e('Product','unilevel-mlm-pro');?> :</td>
...
<?php } ?>
		...
		<tr><td colspan="2">&nbsp;</td></tr>
			<?php   if ( is_plugin_active( 'mlm-paypal-mass-pay/load-data.php' ) ){ ?>
				<tr>
                    <td><?php _e('Paypal ID', 'unilevel-mlm-pro'); ?> <span style="color:red;">*</span> :</td>
