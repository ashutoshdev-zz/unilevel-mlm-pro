	...
	<TABLE id="dataTable"  cellspacing="5" cellpadding="5" border="0" width="450">
		<TR>
			...
			<select name="level[]" id="level" >
				...
				<option value="<?php echo $k; ?>" <?php  echo   (isset($_POST['level']) && $_POST['level']==$k) ? 'selected':''?>><?php _e('Level '.$k,'unilevel-mlm-pro');?></option>
				<?php } ?>
				</select> </TD>
		</TR>
	</TABLE>
...
<?php
	...
	<br\>
		...
		?>
        	<TR>
           		<TD align="left" width="10%"><INPUT type="checkbox" name="chk[]"/></TD>
				<TD align="left" width="40%"> <INPUT type="text" name="per_ref[]" size="15" value="<?php  echo   $mlm_settings['per_ref'][$i]?>"/> </TD>
				<TD align="left" width="25%"> 
			 <INPUT type="text" name="pay_value[]" size="10" value="<?php  echo   $mlm_settings['pay_value'][$i]?>"/> </TD>
				<TD align="left" width="25%"> 
			<select name="level[]" id="level" >
				<option value=""><?php _e('Select Level','unilevel-mlm-pro');?></option> 
				<?php  for($k=1;$k<=$mlm_no_of_level;$k++) { ?>
				<option value="<?php echo $k; ?>" <?php  echo   $mlm_settings['level'][$i]==$k ? 'selected':''?>><?php _e('Level '.$k,'unilevel-mlm-pro');?></option>
				<?php } ?>
				</select> </TD>
