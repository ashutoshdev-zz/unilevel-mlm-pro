...
$mlm_settings = get_option('wp_mlm_general_settings');
	...
	<table width="100%" border="0" cellspacing="10" cellpadding="1" id="payout-page">
		...
		<tr>
			...
			<?php if($id == ''){?>
			<td><a href="<?php  echo  get_post_id_or_postname_for_payout('mlm_my_payout_details_page', $row->payout_id)?>" style="text-decoration:none;"><?php _e('View','unilevel-mlm-pro');?></a></td>
			<?php }
				else{	?>
			<td><a href="?page=mlm-user-account&ac=payout-details&pid=<?php  echo  $row->payout_id?>" style="text-decoration:none;"><?php _e('View','unilevel-mlm-pro');?></a></td>				
				<?php
			}
			?>
