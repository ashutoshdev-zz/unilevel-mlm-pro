...
function mlm_core_add_product_price()
{
	global $wpdb;
    $charset_collate = mlm_core_set_charset();
    $table_prefix = mlm_core_get_table_prefix();
    ...
    if (!in_array('payment_date', $fields)) {
        ...
        $wpdb->query("ALTER TABLE   {$table_prefix}mlm_users ADD payment_date datetime NOT NULL AFTER product_price, {$charset_collate} ");
    }
...
function mlm_core_add_epin_price()
{
	global $wpdb;
    $charset_collate = mlm_core_set_charset();
    $table_prefix = mlm_core_get_table_prefix();
...
function mlm_core_update_hierarchy()
{
    ...
    $results = $wpdb->get_results("desc {$table_prefix}mlm_hierarchy");
    foreach ($results as $key => $value) {
        $fileds[] = $value->Field;
    }
    if (!in_array('slab', $fileds)) {
        $wpdb->query("ALTER TABLE   {$table_prefix}mlm_hierarchy add  slab int(11) NOT NULL DEFAULT '0'");
    }
}
