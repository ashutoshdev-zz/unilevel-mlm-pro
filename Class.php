...
/**
 * Core Class for Unilevel MLM Pro
...
//			$this->pluginURL = plugins_url('', $this->pluginPath);
        ...
        function Plugin_Info_Hook($res, $action, $args) { 
            if ($res === false && $action == 'plugin_information' && $args->slug == 'uni-level-mlm-pro') {
                ...
                $res->requires = "3.0";
                $res->sections = array(
                    'description' => '<p>Unilevel MLM Pro is a powerful solution for creating an online membership site â€“ all built using WordPress as the core content management system.</p>
        ...
        function UpdateNag() {
            if (!$this->Plugin_Is_Latest()) {
                ...
                if (!get_user_meta($user_id, $dismiss_meta) && current_user_can('update_plugins')) {
                    ...
                    printf(__("The most current version of Unilevel MLM Pro is v%s.", 'wishlist-member'), $latest_ump_ver);
                    echo " ";
